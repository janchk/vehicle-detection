...
def vis_all_detection(im_array, detections, class_names, scale, cfg, threshold=1e-3):
    ...
    for j, name in enumerate(class_names):
        ...
        for det in dets:
            ...
            # print(bbox[0])
            cv2.rectangle(im3, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])),color=(127, 0, 255), thickness=2 )
            # cv2.putText(im3, '%s %.3f' % (class_names[j], score), (int(bbox[0]), int(bbox[1]) + 10),
                        # color=color_white, 
                        # fontFace=cv2.FONT_HERSHEY_COMPLEX, 
                        # fontScale=0.5)
            # print(int(bbox[0]), int(bbox[2]) ,  (int(bbox[1]), int(bbox[3])))
    ...
    im2 = np.array(im)
    cv2.imwrite("img_aerial3.png", im3)
    # plt.show()
