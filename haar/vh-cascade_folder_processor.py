...
PATH_TO_VALSET = '../data/valset-norm-size.txt'
...
def process_pic(img_name):
    ...
    cars = vsmall_cascade.detectMultiScale(gray, 1.3, 5)
    hcars = hsmall_cascade.detectMultiScale(gray, 1.3, 5)
    ...
    for (x, y, w, h) in hcars:
        score = i/l
        yield "{} {:.5} {} {} {} {}\n".format(img_name.replace('.png', ''), score, x, y, x + w, y + h)


def process_dir(dir_path=PATH_TO_DATA, filename='ns-results_small-vehicle.txt'):
    now = time.time()
    ...
    for iname in tqdm_notebook(img_names):
        ...
        if ii%20 == 0:
            print('done {}/{} in {} msec'.format(ii, len(desired_files), time.time()-now))
